function [X,outer_errs,outer_res_ln_errs,inner_errs,inner_res_ln_errs] = FacTRBGS_err(U,V,Y,X_0,X_true,X_ln,T,out_b,in_b)
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
    ...
    for t = 1:T
        ...
        % Sample column slice from V to update X in terms of Z residuals
        i_t = randsample(l_2,in_b,false);
        V_slice = V(:,i_t,:);
        I = eye(l_2,l_2);
        E = zeros(l_2,in_b,n);
        E(:,:,1)=I(:,i_t);
