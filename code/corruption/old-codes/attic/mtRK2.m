%version of mtRK where we first restrict the slice subsystem to uncorrupted
%part of residual and then calculate projection there
...
function [X,its] = mtRK2(A,B,X0,T, c_sub1, c_sub2)
    ...
    for t = 1:T
        ...
        %restrict the residual to only uncorrupted vertical slices of X
        p=find(c_sub1 == i_t);
        resid = tprod(A_slice,X(:,setdiff(1:m,c_sub2(p)),:)) - B_slice(:,setdiff(1:m,c_sub2(p)),:);
        %resid = tprod(A_slice,X) - B_slice;
        temp_subtract = tprod(tprod(A_slice_t,A_prod_inv),resid);
        X(:,setdiff(1:m,c_sub2(p)),:) = X(:,setdiff(1:m,c_sub2(p)),:) - temp_subtract;
