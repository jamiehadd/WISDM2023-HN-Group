...
l = 5; p = 4; n = 10; m = 25;
batch_size_set = 1:6; % this is to experiment with different batch sizes
...
disp("Batch sizes: " + mat2str(batch_size_set));
disp("q = " + q);

% Initialization
b_idx = 1;
median_errs_matrix = zeros(length(batch_size_set),num_its+1);
median_time_matrix = zeros(length(batch_size_set),num_its+1);


for batch_size = batch_size_set % sweep batch sizes
    disp("Running with batch size: " + batch_size)
    % Initialization
    ...
    for k = 1:num_trials
        ...
        [~, its, times] = batchQTRK_Algorithm_runtime(A,B, X0, num_its, q, batch_size); % Adjust q as needed
...
% Build legend labels directly from batch_size_set
num_batches = length(batch_size_set);
legendLabels = cell(1, num_batches);
for b = 1:num_batches
    legendLabels{b} = strcat('$T = ', num2str(batch_size_set(b)), '$');
end
markers = {'o', 'x', 's', '^', 'd', 'v'};
...
% Save figure (.fig)
figFileName = fullfile(folderName, ['batchQTRK_exp_', num2str(exp_id), '_vs_iteration.fig']);
savefig(individual_fig, figFileName);

% Save figure (.png)
pngFileName = fullfile(folderName, ['batchQTRK_exp_', num2str(exp_id), '_vs_iteration.png']);
print(gcf, pngFileName, '-dpng', '-r300');  % Adjust resolution as needed

% Save figure (.eps)
epsFileName = fullfile(folderName, ['batchQTRK_exp_', num2str(exp_id), '_vs_iteration.eps']);
...
% Set x-axis limits to cover the slowest batch size's full runtime
xlim([0, max(median_time_matrix(:))]);

set(gcf, 'Position', [100, 100, 900, 600]);  % [left, bottom, width, height]

% Save figure (.fig)
figFileName = fullfile(folderName, ['batchQTRK_exp_', num2str(exp_id), '_vs_runtime.fig']);
savefig(individual_fig, figFileName);

% Save figure (.png)
pngFileName = fullfile(folderName, ['batchQTRK_exp_', num2str(exp_id), '_vs_runtime.png']);
print(gcf, pngFileName, '-dpng', '-r300');  % Adjust resolution as needed

% Save figure (.eps)
epsFileName = fullfile(folderName, ['batchQTRK_exp_', num2str(exp_id), '_vs_runtime.eps']);
print(gcf, epsFileName, '-depsc', '-r300');
...
fprintf(discp, "Batch sizes: %s\n", mat2str(batch_size_set));
