...
[Z,its_X,its_Z] = tRK_t_prod(A2,A1,Y_pad_reorder,X0,10000);
...
%apply tensor regression technique
[Z1,its] = tRK(tprod(A2,A1),Y_pad_reorder,X0,10000);
...
for i = 1:it
    ...
    title(sprintf('facTRK rec, frame%d',i))

    subplot(4,it,i+3*it)
    imagesc(Z1(:,:,i));axis image off;colormap gray
    title(sprintf('TRK rec, frame%d',i))
end